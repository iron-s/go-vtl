...
func TestMap_ContainsKey(t *testing.T) {
	...
	tests := []struct {
		...
		{"key in map exists",
			...
			true},
		{"nil key exists",
			fields{m: map[interface{}]interface{}{nil: "field"}},
			args{key: nil},
			true},
...
func TestMap_Equals(t *testing.T) {
	...
	tests := []struct {
		...
		{"different length the other way",
			...
			false,
			assert.NoError},
		{"different comparable content",
			fields{map[string]interface{}{"a": 1, "and": "string"}},
			args{&Map{map[string]interface{}{"a": 1, "and": "other"}}},
			false,
			assert.NoError},
		{"different non-comparable content",
			fields{map[string]interface{}{"a": 1, "and": []string{"string"}}},
			args{&Map{map[string]interface{}{"a": 1, "and": []string{"other"}}}},
			false,
			assert.NoError},
...
func TestMap_Get(t *testing.T) {
	...
	tests := []struct {
		...
		{"present key",
			...
			2},
		{"nil value",
			fields{map[string]interface{}{"1": nil}},
			args{"1"},
			nil},
	}
	for _, tt := range tests {
...
func TestMap_GetOrDefault(t *testing.T) {
	...
	tests := []struct {
		...
		{"convertible key",
			...
			args{1, 4},
			2},
		{"assignable key exists",
			fields{map[interface{}]interface{}{"1": 2}},
			args{"1", 4},
			2},
		{"assignable key exists",
			fields{map[interface{}]interface{}{"1": 2}},
			...
			4},
		{"nil key exists",
			fields{map[interface{}]interface{}{nil: 2}},
			args{nil, 4},
			2},
	}
	for _, tt := range tests {
...
func TestMap_Put(t *testing.T) {
	...
	tests := []struct {
		...
		{"and adds another",
			fields{m},
			args{"other", "key"},
			nil, "key", assert.NoError, ""},
		{"wrong key type",
			fields{m},
			args{1.0, "key"},
			nil, nil, assert.Error, "cannot convert key float64 to string"},
		{"wrong value type",
			fields{msi},
			args{"other", "string"},
			nil, nil, assert.Error, "cannot convert value string to int"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			m := &Map{
			...
			assert.Equal(t, tt.want, got)
			assert.Equal(t, tt.wantV, m.Get(tt.args.key))
			if tt.assertion(t, err) && tt.wantErr != "" {
...
func TestMap_PutAll(t *testing.T) {
	...
	tests := []struct {
		...
		{"wrong key type",
			fields{m},
			args{&Map{map[float32]interface{}{1.0: "b", 2.0: 2}}},
			map[string]interface{}{},
			assert.Error},
		{"wrong value type",
			fields{msi},
			args{&Map{map[string]string{"a": "string", "c": "other"}}},
			map[string]int{},
			assert.Error},
		{"works on empty map",
			fields{m},
		...
		{"adds and replaces value",
			fields{m},
			args{&Map{map[string]interface{}{"c": "d", "e": []int{1}, "n": nil}}},
			map[string]interface{}{"a": "b", "c": "d", "e": []int{1}, "n": nil},
			assert.NoError},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestMap_Replace(t *testing.T) {
	...
	tests := []struct {
		...
		{"replaces with nil",
			...
			args{"other", nil},
			"key",
			map[string]interface{}{"k": 1, "other": nil}},
		{"replaces some other",
			fields{map[string]interface{}{"k": 1, "other": "key"}},
			args{"other", "new"},
			"key",
...
func TestMap_Values(t *testing.T) {
	...
	empty := emptyMap()
	m := map[string]interface{}{"second": "value", "first": []int{1}, "nil": nil}
	tests := []struct {
		name   string
		fields fields
		want   *ValView
...
func TestMapEntry_SetValue(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.want, got)
			tt.assertion(t, err)
			assert.Equal(t, tt.wantM, tt.fields.m.m)
		})
	}
}
...
func TestValView_Iterator(t *testing.T) {
	...
	tests := []struct {
		...
		{"multiple values in order",
			fields{&Map{map[string]interface{}{"b": 1, "a": "value", "c": 1, "d": nil}}},
			[]interface{}{"value", 1, 1, nil}},
		{"typed map",
...
func TestValView_ToArray(t *testing.T) {
	...
	tests := []struct {
		...
		{"nil values should be supported",
			fields{&Map{map[string]interface{}{"b": 1, "a": nil}}},
			&Slice{[]interface{}{nil, 1}}, assert.NoError},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			view := &ValView{
...
func TestValView_RemoveAll(t *testing.T) {
	...
	tests := []struct {
		...
		{"removes from slice, map, and key slice",
			...
			map[string]interface{}{"key": "value"}},
		{"removes given range",
			fields{&Map{map[int]interface{}{1: 1, 2: 2}}},
			args{&Range{2, 5, 1}},
			true,
			assert.NoError,
			map[int]interface{}{1: 1}},
		{"removes even non-comparable values",
			fields{&Map{map[string]interface{}{"k": []int{1}, "key": "value"}}},
...
func TestSlice_ToArray(t *testing.T) {
	...
	tests := []struct {
		...
		{"works with nil values",
			fields{[]interface{}{1, nil, 2}},
			&Slice{[]interface{}{1, nil, 2}}},
		{"returns copy",
			fields{[]interface{}{1, "2", []int{3}}},
...
func TestNewRange(t *testing.T) {
	...
	tests := []struct {
		...
		{"single element", args{0, 0}, &Range{0, 0, 1}, nil},
		{"up positive", args{0, 5}, &Range{0, 5, 1}, nil},
		{"down positive", args{5, 0}, &Range{5, 0, -1}, nil},
		{"up negative", args{-5, -3}, &Range{-5, -3, 1}, nil},
		{"down negative", args{-3, -5}, &Range{-3, -5, -1}, nil},
		{"up cross", args{-3, 5}, &Range{-3, 5, 1}, nil},
		{"down cross", args{3, -5}, &Range{3, -5, -1}, nil},
		{"max len 1", args{0, 9223372036854775806}, &Range{0, 9223372036854775806, 1}, nil},
		{"max len 2", args{9223372036854775806, 0}, &Range{9223372036854775806, 0, -1}, nil},
		{"max len 3", args{0, -9223372036854775806}, &Range{0, -9223372036854775806, -1}, nil},
		{"max len 4", args{-9223372036854775806, 0}, &Range{-9223372036854775806, 0, 1}, nil},
		{"max len 5", args{-4611686018427387903, 4611686018427387903}, &Range{-4611686018427387903, 4611686018427387903, 1}, nil},
		{"max len 6", args{4611686018427387903, -4611686018427387903}, &Range{4611686018427387903, -4611686018427387903, -1}, nil},
		{"max len 7", args{-9223372036854775807, -1}, &Range{-9223372036854775807, -1, 1}, nil},
		{"overflow 1", args{0, 9223372036854775807}, nil, errRangeOverflow},
		{"overflow 2", args{9223372036854775807, 0}, nil, errRangeOverflow},
		{"overflow 3", args{-4611686018427387904, 4611686018427387903}, nil, errRangeOverflow},
		{"overflow 4", args{4611686018427387904, -4611686018427387903}, nil, errRangeOverflow},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			r, err := NewRange(tt.args.start, tt.args.end)
...
func TestNewMapIterator(t *testing.T) {
	...
	var (
		...
		// mapInterface = map[interface{}]interface{}{1: 1, 2: 2}
	)
	val := reflect.ValueOf
	tests := []struct {
		name string
		...
		// FIXME make interface work as a key
		// {"map[interface{}]interface{}", args{&Map{mapInterface}, nil},
		// 	&MapIterator{mapper: nil, i: 0, last: 0,
		// 		mM: val(mapInterface),
		// 		k:  []reflect.Value{val(1), val(2)}}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
